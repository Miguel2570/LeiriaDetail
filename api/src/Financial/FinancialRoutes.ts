...
async function GetFinancialData(request: Request, response: Response) {
    try {
        ...
        const [summary, revenue, transactions] = await Promise.all([
            FinancialManager.getSummary(period),
            FinancialManager.getRevenueChart(period),
            FinancialManager.getTransactions(20)
        ]);
        
        response.status(200).json(
            new FinancialOutputModel(summary, revenue, transactions, "Dados financeiros carregados.")
        );
    } catch (error: any) {
        console.error('Financial error:', error);
        response.status(500).json(
            new FinancialOutputModel(undefined, undefined, undefined, undefined, 
                new ErrorModel("Server", "Erro ao carregar dados financeiros."))
        );
    }
}
...
async function GetSummary(request: Request, response: Response) {
    try {
        const period = (request.query.period as string) || '7d';
        const summary = await FinancialManager.getSummary(period);
        
        response.status(200).json(
            new FinancialOutputModel(summary, undefined, undefined, "Resumo carregado.")
        );
    } catch (error: any) {
        response.status(500).json(
            new FinancialOutputModel(undefined, undefined, undefined, undefined, 
                new ErrorModel("Server", "Erro ao carregar resumo."))
        );
    }
}
...
async function GetTransactions(request: Request, response: Response) {
    try {
        ...
        response.status(200).json(
            new FinancialOutputModel(undefined, undefined, transactions, "Transações carregadas.")
        );
    } catch (error: any) {
        response.status(500).json(
            new FinancialOutputModel(undefined, undefined, undefined, undefined, 
                new ErrorModel("Server", "Erro ao carregar transações."))
        );
    }
}
...
async function CreateTransaction(request: Request, response: Response) {
    try {
        ...
        response.status(201).json(
            new FinancialOutputModel(undefined, undefined, [result.rows[0]], "Transação criada.")
        );
    } catch (error: any) {
