      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-20">
        ...
        >
          ...
          <div class="relative z-10">
            ...
            <!-- ✅ Link para detalhes (usa ID real do serviço) -->
            <router-link 
              :to="`/servicos/${getServiceId(cat.id)}`" 
              class="text-xs text-[#00D8FF] hover:underline font-bold inline-flex items-center gap-1"
            >
              Ver detalhes → 
            </router-link>
          </div>
...
const mainCategories = [
  { id: 'exterior', title: 'Lavagem Exterior', subtitle: 'Brilho e Proteção', icon: CarFront, description: 'Lavagem manual segura com produtos pH neutro. Devolvemos o brilho original à pintura.', features: ['Pré-lavagem com espuma ativa', 'Método dos 2 baldes', 'Secagem com ar quente', 'Selagem de pintura'], image: '🧼' },
  { id: 'interior', title: 'Detalhe Interior', subtitle: 'Habitáculo Imaculado', icon: Shield, description: 'Limpeza profunda de todos os materiais. O seu carro com aspeto e cheiro a novo.', features: ['Aspiração profunda', 'Lavagem de estofos', 'Hidratação de peles', 'Higienização com Ozono'], image: '🧹' },
  { id: 'completo', title: 'Tratamento Completo', subtitle: 'A Experiência Premium', icon: Sparkles, description: 'O pacote completo para quem exige o melhor. Exterior, interior e proteção cerâmica.', features: ['Correção de pintura', 'Proteção cerâmica 9H', 'Detalhe interior completo', 'Garantia de satisfação'], image: '👑' },
  { id: 'jantes', title: 'Jantes e Pneus', subtitle: 'Detalhe Especializado', icon: Droplets, description: 'Limpeza e proteção das jantes, pneus e discos. O detalhe que faz a diferença.', features: ['Descontaminação química', 'Limpeza face e interior', 'Selagem protetora', 'Brilho espelhado'], image: '💎' }
];

const totalServices = ref(0);
const packs = ref<string[]>([]);
const services = ref<any[]>([]); // ✅ NOVO

// ✅ Mapeamento de categorias para nomes de serviços
const categoryMapping: Record<string, string> = {
  'exterior': 'Lavagem Exterior',
  'interior': 'Lavagem Interior',
  'completo': 'Lavagem Completa',
  'jantes': 'Jantes'
};

// ✅ Obter ID real do serviço pela categoria
const getServiceId = (category: string): string => {
  const searchName = categoryMapping[category] || category;
  const svc = services.value.find(s => 
...
const fetchStats = async () => {
  try {
    const query = `query { services { services { id name packType } } }`;
