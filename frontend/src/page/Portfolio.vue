    ...
    <div class="container mx-auto px-4 relative z-10">
      ...
      <template v-else>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-7xl mx-auto">
          <div v-for="p in filtered" :key="p.id" class="group relative overflow-hidden rounded-[2.5rem] bg-[#0A0A0F] aspect-[4/3] border border-white/5">
            <img :src="p.imageUrl" :alt="p.title" class="w-full h-full object-cover opacity-60 group-hover:opacity-80 group-hover:scale-110 transition-all duration-700" />
