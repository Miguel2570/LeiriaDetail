  ...
  <div class="min-h-screen bg-[#020204] text-white pt-28 pb-20 relative overflow-hidden">
    ...
    <div class="max-w-6xl mx-auto px-4 relative z-10">
      ...
      <div class="mb-8 border-b border-white/5 pb-8">
        <h2 class="text-[#00D8FF] font-black tracking-[0.4em] uppercase text-[10px] mb-2 italic">
          {{ t('clientArea.title') }}
        </h2>
        <h1 class="text-4xl font-black italic tracking-tighter uppercase text-white">
          {{ t('clientArea.greeting') }}, 
          <span class="bg-gradient-to-r from-[#2563EB] to-[#00D8FF] bg-clip-text text-transparent">
            {{ customerName }}
          </span>
        </h1>
      </div>
        ...
        <div class="flex-1 min-w-0">
            ...
            <div class="flex items-center gap-3">
              ...
              <div>
                ...
                <p class="text-gray-400 text-xs">
                  {{ pendingBooking.serviceName }} • {{ pendingBooking.vehicleName }} • {{ formatPendingDate(pendingBooking.date) }}
                </p>
              </div>
            </div>
            <button 
              @click="continuePayment" 
              class="px-4 py-2 bg-amber-500/20 text-amber-400 font-bold rounded-xl text-sm hover:bg-amber-500/30 transition-all flex-shrink-0"
            >
              Concluir Pagamento →
            </button>
          </div>
          ...
          <div v-if="showReturnToBookingBanner" class="bg-blue-500/10 border border-blue-500/30 rounded-2xl p-4 mb-6 flex items-center justify-between">
            ...
            <button 
              ...
              class="px-4 py-2 bg-blue-500/20 text-blue-400 font-bold rounded-xl text-sm hover:bg-blue-500/30 transition-all flex-shrink-0"
            >
          ...
          <div v-if="activeTab === 'profile'" class="space-y-8">
            <div class="bg-[#050508] border border-white/10 rounded-2xl p-6">
              <h3 class="text-lg font-bold text-white mb-6 flex items-center gap-2">
                <User class="w-5 h-5 text-[#00D8FF]" /> 
                {{ t('clientArea.profile.personalData') }}
              </h3>
              ...
              <form @submit.prevent="handleUpdateProfile" class="space-y-4">
                ...
                <button 
                  ...
                  class="px-6 py-3 bg-gradient-to-r from-[#2563EB] to-[#00D8FF] text-white text-xs font-bold uppercase tracking-wider rounded-xl disabled:opacity-50"
                >
            ...
            <div class="bg-[#050508] border border-white/10 rounded-2xl p-6">
              <h3 class="text-lg font-bold text-white mb-6 flex items-center gap-2">
                <Lock class="w-5 h-5 text-[#00D8FF]" /> 
                {{ t('clientArea.profile.changePassword') }}
              </h3>
              ...
              <button 
                ...
                class="px-6 py-3 bg-[#00D8FF]/10 border border-[#00D8FF]/30 text-[#00D8FF] text-xs font-bold uppercase tracking-wider rounded-xl hover:bg-[#00D8FF]/20 disabled:opacity-50"
              >
                {{ passwordSubmitting ? t('clientArea.profile.changing') : t('clientArea.profile.change') }}
              </button>
              <p v-if="passwordMessage" class="text-xs mt-2" :class="passwordError ? 'text-red-400' : 'text-green-400'">
                {{ passwordMessage }}
              </p>
            </div>
            ...
            <div class="bg-[#050508] border border-white/10 rounded-2xl p-6">
              <h3 class="text-lg font-bold text-white mb-4 flex items-center gap-2">
                <ShieldAlert class="w-5 h-5 text-[#00D8FF]" /> 
                {{ t('clientArea.profile.account') }}
              </h3>
            ...
            <div class="bg-[#050508] border border-red-500/20 rounded-2xl p-6">
              ...
              <p class="text-sm text-gray-400 mb-4">
                Ao apagar a sua conta, todos os dados pessoais serão anonimizados. As marcações anteriores serão mantidas por obrigação fiscal, mas deixarão de estar associadas a si.
              </p>
              <button 
              ...
              <p v-if="deleteMessage" class="text-xs mt-2" :class="deleteError ? 'text-red-400' : 'text-green-400'">
                {{ deleteMessage }}
              </p>
            </div>
          </div>
          ...
          <div v-if="activeTab === 'vehicles'" class="bg-[#050508] border border-white/10 rounded-2xl p-6">
            <div class="flex justify-between items-center mb-6">
              <h3 class="text-lg font-bold text-white uppercase tracking-wider flex items-center gap-2">
                <Car class="w-5 h-5 text-[#00D8FF]" /> 
                {{ t('clientArea.vehicles.title') }}
              </h3>
              <button 
                @click="showAddVehicleForm = !showAddVehicleForm" 
                class="px-4 py-2 bg-gradient-to-r from-[#2563EB] to-[#00D8FF] text-white text-xs font-bold rounded-xl"
              >
                <Plus class="w-4 h-4 inline mr-1" /> 
                {{ showAddVehicleForm ? t('clientArea.vehicles.cancel') : t('clientArea.vehicles.add') }}
              </button>
            </div>

            <Transition name="slide">
              <div v-if="showAddVehicleForm" class="mb-6 p-6 border border-white/10 rounded-xl bg-white/[0.02]">
                <div v-if="vehicleError" class="mb-4 p-3 bg-red-500/10 border border-red-500/50 rounded-xl text-red-400 text-xs">
                  {{ vehicleError }}
                </div>
                ...
                <form @submit.prevent="handleAddVehicle" class="space-y-4">
                  <div>
                    <label class="block text-xs font-bold text-gray-400 uppercase mb-2">
                      {{ t('clientArea.vehicles.plate') }} *
                    </label>
                    <input 
                      v-model="newVehicle.license_plate" 
                      type="text" 
                      placeholder="AA-11-BB" 
                      class="w-full px-4 py-3 border rounded-xl text-white outline-none" 
                  ...
                  <div class="grid grid-cols-2 gap-4">
                    <div>
                      <label class="block text-xs font-bold text-gray-400 uppercase mb-2">{{ t('clientArea.vehicles.brand') }} *</label>
                      <input v-model="newVehicle.brand" type="text" class="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white outline-none" required />
                    </div>
                    <div>
                      <label class="block text-xs font-bold text-gray-400 uppercase mb-2">{{ t('clientArea.vehicles.model') }} *</label>
                      <input v-model="newVehicle.model" type="text" class="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white outline-none" required />
                    </div>
                  </div>
                  
                  <div class="grid grid-cols-3 gap-4">
                    <div>
                      <label class="block text-xs font-bold text-gray-400 uppercase mb-2">{{ t('clientArea.vehicles.year') }}</label>
                      <input v-model="newVehicle.year" type="number" class="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white outline-none" />
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <select v-model="newVehicle.size_category" class="w-full px-4 py-3 bg-[#1a1a1a] border border-gray-700 rounded-xl text-white outline-none">
                        ...
                        <option value="Medium">Large (SUV)</option>
                        <option value="Large">Extra Large (Van)</option>
                      </select>
                    </div>
                  </div>
                  ...
                  <button 
                    type="submit" 
                    :disabled="vehicleSubmitting || !plateStatus.isValid" 
            ...
            <div v-if="vehicles.length > 0" class="space-y-3">
              <div 
                ...
                :class="v.isPrimary ? 'border-[#00D8FF]/30 bg-[#00D8FF]/5' : 'border-white/5 bg-white/[0.01]'"
              >
                <div class="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <span v-if="v.isPrimary" class="text-[10px] bg-[#00D8FF]/20 text-[#00D8FF] px-2 py-0.5 rounded-full ml-2">
                      {{ t('clientArea.vehicles.primary') }}
                    </span>
                    <p class="text-xs text-gray-400">{{ v.licensePlate }} • {{ v.year || 'N/D' }}</p>
                  </div>
                </div>
                <div class="flex gap-2">
                  <button v-if="!v.isPrimary" @click="handleSetPrimary(v.id)" class="text-[10px] uppercase font-bold text-gray-400 hover:text-[#00D8FF]">
                    {{ t('clientArea.vehicles.setPrimary') }}
                  </button>
                  <button @click="handleDeleteVehicle(v.id)" class="text-[10px] uppercase font-bold text-red-400 hover:text-red-300">
                    {{ t('clientArea.vehicles.remove') }}
                  </button>
                </div>
              </div>
            </div>
            <div v-else class="text-center py-12">
              <Car class="w-12 h-12 text-gray-600 mx-auto mb-3" />
              <p class="text-gray-400 text-sm">{{ t('clientArea.vehicles.empty') }}</p>
            </div>
          </div>
          ...
          <div v-if="activeTab === 'bookings'">
            ...
            <div v-else-if="bookings.length === 0" class="bg-[#050508] border border-white/10 rounded-2xl p-12 text-center">
              ...
              <router-link to="/servicos" class="text-[#00D8FF] text-sm font-bold hover:underline">
                {{ t('clientArea.bookings.scheduleNow') }} →
              </router-link>
            </div>
            <div v-else class="space-y-4">
              <div v-for="b in bookings" :key="b.id" class="bg-[#050508] border border-white/10 rounded-2xl p-6 hover:border-white/20 transition-all">
                <div class="flex justify-between items-start mb-3">
                  ...
                  <span :class="['text-[10px] font-black uppercase px-3 py-1.5 rounded-full', b.status === 'PENDENTE' ? 'bg-amber-500/10 text-amber-500' : 'bg-emerald-500/10 text-emerald-500']">
                    ● {{ b.status }}
                  </span>
                </div>
          ...
          <div v-if="activeTab === 'history'" class="space-y-6">
            <div class="flex justify-between items-center">
              <h3 class="text-lg font-bold text-white uppercase tracking-wider flex items-center gap-2">
                <History class="w-5 h-5 text-[#00D8FF]" /> 
                {{ t('clientArea.history.title') }}
              </h3>
              <button 
                v-if="history.length > 0" 
                @click="generatePDF" 
                class="px-4 py-2 bg-red-500/10 border border-red-500/30 text-red-400 text-xs font-bold uppercase tracking-wider rounded-xl hover:bg-red-500/20 transition-all flex items-center gap-2"
              >
                <FileText class="w-4 h-4" /> 
                {{ t('clientArea.history.generatePDF') }}
              </button>
            </div>
            ...
            <div v-if="history.length > 0" class="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div class="bg-[#050508] border border-white/10 rounded-xl p-4 text-center">
                <p class="text-2xl font-black text-[#00D8FF] shrink-0 truncate px-1">{{ favoriteService }}</p>
            ...
            <div v-else class="relative">
              ...
              <div v-for="(item, index) in history" :key="item.id" class="relative pl-14 pb-6">
                <div 
                  class="absolute left-3.5 top-1 w-3 h-3 rounded-full border-2" 
                  :class="index === 0 ? 'bg-[#00D8FF] border-[#00D8FF]' : 'bg-[#020204] border-white/20'"
                ></div>
                ...
                <div class="bg-[#050508] border border-white/10 rounded-2xl p-5 hover:border-white/20 transition-all">
                    ...
                    <div>
                      <div class="flex items-center gap-2 mb-1">
                        ...
                        <span 
                          ...
                          :class="item.serviceName.includes('Premium') ? 'bg-[#00D8FF]/20 text-[#00D8FF]' : 'bg-gray-500/20 text-gray-400'"
                        >
                          {{ item.serviceName.includes('Premium') ? 'PREMIUM' : t('clientArea.history.basic') }}
                        </span>
                      </div>
                      <p class="text-xs text-gray-400">
                        🚗 {{ item.vehicleName }} • <span class="font-mono">{{ item.vehiclePlate }}</span>
                      </p>
                    </div>
                  ...
                  <div class="flex flex-wrap items-center gap-4 text-xs text-gray-500">
                    ...
                    <span class="px-2 py-0.5 rounded-full bg-green-500/10 text-green-400 text-[10px] font-bold uppercase">
                      ✅ {{ item.status }}
                    </span>
                  </div>
                </div>
...
import { 
  Calendar, Clock, Sparkles, CalendarPlus, ShieldCheck, Car, Plus, 
  User, Mail, Phone, Lock, ShieldAlert, History, FileText, Shield, 
  Trash2, Gift 
...
const newVehicle = ref({ license_plate: '', brand: '', model: '', year: null as number | null, fuel_type: '', size_category: '' });
const plateStatus = reactive({ isValid: false, class: 'bg-white/5 border-white/10 focus:border-[#3B82F6]', message: '', messageClass: '' });

const pageReady = ref(false);
const pageError = ref('');

// Fluxo de Pagamentos Pendentes
...
const formatPendingDate = (date: any) => { 
  if (!date) return ''; 
  try { 
...
const uniqueVehicles = computed(() => { 
  const plates = new Set(history.value.map(item => item.vehiclePlate)); 
  return plates.size; 
});
...
// Ações do Histórico
const generatePDF = () => { 
  generateHistoryPDF({ name: `${profile.firstName} ${profile.lastName}`, email: profile.email, phone: profile.phone }, history.value); 
};

// Carregamento de Dados Remotos / Mutações
...
const loadHistory = async () => {
  ...
  try {
    ...
    history.value = response.userBookings
      .filter(b => b.status === 'CONCLUIDO' || b.status === 'APROVADO')
      .map(b => ({ ...b, pack_type: b.serviceName.includes('Premium') ? 'Premium' : 'Básico' }))
...
// Tratamento de Inputs & Criação de Veículos
...
const handleSetPrimary = async (vehicleId: string) => { 
  try { 
    await graphql(`mutation { setPrimaryVehicle(vehicleId: "${vehicleId}") { hasError } }`); 
    await loadVehicles(); 
  } catch (error) { 
    console.error(error); 
  } 
...
const handleDeleteVehicle = async (vehicleId: string) => { 
  ...
  try { 
    const data = await graphql<{ deleteVehicle: { success: boolean } }>(`mutation { deleteVehicle(vehicleId: "${vehicleId}") { success } }`); 
    if (data.deleteVehicle.success) await loadVehicles(); 
  } catch (error) { 
    console.error(error); 
  } 
};
