    ...
    <div class="flex flex-col lg:flex-row gap-8 flex-1 overflow-hidden">
      ...
      <div v-if="selectedClient" class="w-full lg:w-2/3 flex flex-col overflow-y-auto space-y-6 pr-2">
        ...
        <div class="p-6 border border-black/10 rounded-2xl backdrop-blur-sm card-inner">
          <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-6">
            <div class="flex items-center gap-5">
              <div class="w-20 h-20 rounded-2xl bg-gradient-to-tr from-[#3B82F6] to-[#06B6D4] flex items-center justify-center shadow-lg text-white text-3xl font-bold">
        ...
        <div>
          <div class="flex items-center justify-between mb-4">
            <h3 class="text-xl font-bold text-[#000000] flex items-center gap-2">
              <Shield class="w-6 h-6 text-[#3B82F6]" /> Client Fleet
            </h3>
            <button @click="setIsAddVehicleOpen(true)" class="text-sm font-bold text-[#06B6D4] hover:text-[#0284C7] flex items-center gap-1 transition-colors">
        ...
        <div class="p-8 border border-black/10 rounded-2xl backdrop-blur-sm card-inner">
          <h4 class="font-bold text-[#000000] text-xl mb-6 flex items-center gap-3">
            <History class="w-6 h-6 text-[#06B6D4]" /> Unified Service History
          </h4>
          <div v-if="selectedClient.history && selectedClient.history.length > 0" class="relative border-l-2 border-[#06B6D4]/30 ml-4 space-y-8 pb-4">
            <div v-for="record in selectedClient.history" :key="record.id" class="relative pl-8">
              <div :class="['absolute -left-[9px] top-1 w-4 h-4 rounded-full border-2 border-white shadow-sm',
                record.status === 'EM_PROGRESSO' ? 'bg-[#06B6D4]' :
                record.status === 'CONCLUIDO' ? 'bg-[#10B981]' : 'bg-[#94A3B8]']" />
              <div class="bg-white/60 p-4 rounded-xl border border-black/5">
                <div class="flex justify-between items-start gap-2 mb-2">
                  <h5 class="font-bold text-[#0F172A]">{{ record.service }}</h5>
                  <span :class="['px-2.5 py-1 rounded-full font-bold text-[10px] uppercase tracking-wide whitespace-nowrap',
                    record.status === 'CONCLUIDO' ? 'bg-[#D1FAE5] text-[#059669]' :
                    record.status === 'EM_PROGRESSO' ? 'bg-[#E0F2FE] text-[#0284C7]' : 'bg-[#F1F5F9] text-[#64748B]']">
                    {{ record.status }}
                  </span>
                </div>
      ...
      <div v-else class="w-full lg:w-2/3 flex items-center justify-center text-[#64748B] font-medium">
      ...
      <div class="w-full max-w-sm bg-white/90 backdrop-blur-xl border border-white/50 p-8 rounded-2xl shadow-2xl">
        ...
        <form @submit.prevent="handleAddClient" class="space-y-4">
          <div class="grid grid-cols-2 gap-4">
            <div>
              <label class="block text-sm font-bold text-[#334155] mb-1">First Name</label>
              <input required v-model="newClient.firstName" class="w-full px-4 py-3 rounded-xl bg-white/60 border border-[#06B6D4]/30 focus:outline-none focus:ring-2 focus:ring-[#06B6D4]/20 text-[#000000] font-medium" />
            </div>
            <div>
              <label class="block text-sm font-bold text-[#334155] mb-1">Last Name</label>
              <input required v-model="newClient.lastName" class="w-full px-4 py-3 rounded-xl bg-white/60 border border-[#06B6D4]/30 focus:outline-none focus:ring-2 focus:ring-[#06B6D4]/20 text-[#000000] font-medium" />
            </div>
          </div>
          <div>
            <label class="block text-sm font-bold text-[#334155] mb-1">Email</label>
            <input required type="email" v-model="newClient.email" class="w-full px-4 py-3 rounded-xl bg-white/60 border border-[#06B6D4]/30 focus:outline-none focus:ring-2 focus:ring-[#06B6D4]/20 text-[#000000] font-medium" />
          </div>
          <div>
            <label class="block text-sm font-bold text-[#334155] mb-1">Phone</label>
            <input v-model="newClient.phone" class="w-full px-4 py-3 rounded-xl bg-white/60 border border-[#06B6D4]/30 focus:outline-none focus:ring-2 focus:ring-[#06B6D4]/20 text-[#000000] font-medium" />
          </div>
          <button type="submit" :disabled="isSubmitting" class="w-full mt-6 py-4 bg-gradient-to-r from-[#3B82F6] to-[#06B6D4] text-white rounded-xl font-bold shadow-[0_0_12px_rgba(59,130,246,0.4)] hover:shadow-[0_0_20px_rgba(59,130,246,0.6)] transition-all disabled:opacity-50">
            {{ isSubmitting ? 'A criar...' : 'Create Client' }}
          </button>
        </form>
      </div>
    </div>
    ...
    <div v-if="isAddVehicleOpen" class="absolute inset-0 z-50 flex items-center justify-center bg-black/20 backdrop-blur-sm rounded-[16px]">
      <div class="w-full max-w-sm bg-white/90 backdrop-blur-xl border border-white/50 p-8 rounded-2xl shadow-2xl">
        ...
        <form @submit.prevent="handleAddVehicle" class="space-y-4">
          <div>
            <label class="block text-sm font-bold text-[#334155] mb-1">License Plate</label>
            <input required v-model="newVehicle.plate" class="w-full px-4 py-3 rounded-xl bg-white/60 border border-[#06B6D4]/30 focus:outline-none focus:ring-2 focus:ring-[#06B6D4]/20 text-[#000000] font-medium uppercase" />
          </div>
          <div class="grid grid-cols-2 gap-4">
            <div>
              <label class="block text-sm font-bold text-[#334155] mb-1">Brand</label>
              <input required v-model="newVehicle.brand" class="w-full px-4 py-3 rounded-xl bg-white/60 border border-[#06B6D4]/30 focus:outline-none focus:ring-2 focus:ring-[#06B6D4]/20 text-[#000000] font-medium" />
            </div>
            <div>
              <label class="block text-sm font-bold text-[#334155] mb-1">Model</label>
              <input required v-model="newVehicle.model" class="w-full px-4 py-3 rounded-xl bg-white/60 border border-[#06B6D4]/30 focus:outline-none focus:ring-2 focus:ring-[#06B6D4]/20 text-[#000000] font-medium" />
            </div>
          </div>
          <div class="grid grid-cols-2 gap-4">
            <div>
              <label class="block text-sm font-bold text-[#334155] mb-1">Year</label>
              <input required type="number" v-model="newVehicle.year" class="w-full px-4 py-3 rounded-xl bg-white/60 border border-[#06B6D4]/30 focus:outline-none focus:ring-2 focus:ring-[#06B6D4]/20 text-[#000000] font-medium" />
            </div>
            <div>
              <label class="block text-sm font-bold text-[#334155] mb-1">Size</label>
              <select v-model="newVehicle.sizeCategory" class="w-full px-4 py-3 rounded-xl bg-white/60 border border-[#06B6D4]/30 focus:outline-none focus:ring-2 focus:ring-[#06B6D4]/20 text-[#000000] font-medium">
          ...
          <button type="submit" :disabled="isSubmitting" class="w-full mt-6 py-4 bg-gradient-to-r from-[#3B82F6] to-[#06B6D4] text-white rounded-xl font-bold shadow-[0_0_12px_rgba(59,130,246,0.4)] hover:shadow-[0_0_20px_rgba(59,130,246,0.6)] transition-all disabled:opacity-50">
            {{ isSubmitting ? 'A adicionar...' : 'Add Vehicle' }}
          </button>
        </form>
      </div>
    </div>
...
const handleAddVehicle = async () => {
  ...
  isSubmitting.value = true
  try {
    const response = await fetch('/CRM/Vehicle', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        userId: selectedClientId.value,
        ...newVehicle.value
      })
    })
    const data = await response.json()
