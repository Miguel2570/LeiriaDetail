    ...
    <div v-else class="flex flex-col lg:flex-row gap-8 flex-1 overflow-hidden">
      ...
      <div class="w-full lg:w-1/3 flex flex-col bg-white/40 border border-[#06B6D4]/30 rounded-2xl p-5 overflow-y-auto backdrop-blur-md">
        <div class="flex items-center justify-between mb-4 pb-4 border-b border-[#06B6D4]/20">
          <h4 class="font-bold text-[#000000] text-xl flex items-center gap-2">
            <span class="w-3 h-3 rounded-full bg-[#F59E0B] animate-pulse" /> Requests
          </h4>
        ...
        <div v-else class="space-y-4">
          <div v-for="booking in pending" :key="booking.id" draggable="true" @dragstart="handleDragStart($event, booking)"
      ...
      <div class="w-full lg:w-2/3 grid grid-cols-1 md:grid-cols-3 gap-6 overflow-y-auto pr-2 pb-4">
        <div v-for="bay in bays" :key="bay.id" @drop="handleDrop($event, bay.id)" @dragover.prevent
      ...
      <div class="w-full max-w-lg bg-white/90 backdrop-blur-xl border border-white/50 p-8 rounded-2xl shadow-2xl">
        ...
        <form @submit.prevent="handleAddBooking" class="space-y-4">
          <div class="grid grid-cols-2 gap-4">
            <div>
              <label class="block text-sm font-bold text-[#334155] mb-1">User ID</label>
              <input required type="number" v-model="form.userId" class="w-full px-4 py-2.5 rounded-xl bg-white/60 border border-[#06B6D4]/30 focus:outline-none focus:ring-2 focus:ring-[#06B6D4]/20 text-[#000000] font-medium" />
            </div>
            <div>
              <label class="block text-sm font-bold text-[#334155] mb-1">Vehicle ID</label>
              <input required type="number" v-model="form.vehicleId" class="w-full px-4 py-2.5 rounded-xl bg-white/60 border border-[#06B6D4]/30 focus:outline-none focus:ring-2 focus:ring-[#06B6D4]/20 text-[#000000] font-medium" />
            </div>
          </div>
          <div>
            <label class="block text-sm font-bold text-[#334155] mb-1">Service ID</label>
            <input required type="number" v-model="form.serviceId" class="w-full px-4 py-2.5 rounded-xl bg-white/60 border border-[#06B6D4]/30 focus:outline-none focus:ring-2 focus:ring-[#06B6D4]/20 text-[#000000] font-medium" />
          </div>
          <div class="grid grid-cols-2 gap-4">
            <div>
              <label class="block text-sm font-bold text-[#334155] mb-1">Date</label>
              <input required type="date" v-model="form.bookingDate" class="w-full px-4 py-2.5 rounded-xl bg-white/60 border border-[#06B6D4]/30 focus:outline-none focus:ring-2 focus:ring-[#06B6D4]/20 text-[#000000] font-medium" />
            </div>
            <div>
              <label class="block text-sm font-bold text-[#334155] mb-1">Time</label>
              <input required type="time" v-model="form.bookingTime" class="w-full px-4 py-2.5 rounded-xl bg-white/60 border border-[#06B6D4]/30 focus:outline-none focus:ring-2 focus:ring-[#06B6D4]/20 text-[#000000] font-medium" />
            </div>
          </div>
...
const fetchAppointments = async () => {
  try {
    ...
    if (!data.HasError && data.Data) {
      ...
      // Distribuir bookings pelos bays
      if (data.Data.bays) {
        bays.value = data.Data.bays.map((b: any) => ({
          ...b,
          bookings: b.bookings || []
        }))
      }
